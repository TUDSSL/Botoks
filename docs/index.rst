...
+------------+---------+-----------------------------------------------------------------------------------------------------------+
| CMake      | 3.13    | `download <https://cmake.org/download/>`__                                                                |
+------------+---------+-----------------------------------------------------------------------------------------------------------+
| MSP430-GCC | 8.3.0   | `download <http://software-dl.ti.com/msp430/msp430_public_sw/mcu/msp430/MSPGCC/latest/index_FDS.html>`_   |
+------------+---------+-----------------------------------------------------------------------------------------------------------+
| UniFlash   | 5.2.0   | `download <http://www.ti.com/tool/UNIFLASH>`__                                                            |
+------------+---------+-----------------------------------------------------------------------------------------------------------+
...
**without** root privileges (check `here <https://docs.docker.com/install/linux/linux-postinstall/#manage-docker-as-a-non-root-user>`_).  The script uses
`this docker image <https://hub.docker.com/r/cdelledonne/msp430-gcc>`_ to build applications inside a Docker
container pre-configured with CMake and the MSP430-GCC toolchain.

You can pass ``-t <target>`` to the script to specify a target for ``make``.
...
This will download toolchain and other support files from `TI's website <http://software-dl.ti.com/msp430/msp430_public_sw/mcu/msp430/MSPGCC/latest/index_FDS.html>`__ , and
...
To upload an application install `UniFlash <http://www.ti.com/tool/UNIFLASH>`_.  As of now, all scripts
...
`picocom <https://linux.die.net/man/8/picocom>`__) with a baudrate of 19200.
...
<chrt-calibration>`.  Running the calibration procedure requires a licensed
version of `Matlab <https://www.mathworks.com/products/matlab.html>`_ and the serial terminal ``picocom``.
...
:doc:`chrt`.  Then, checkout the :doc:`applications <applications>`.  Have a
look at the :doc:`hardware implementation <hardware>` as well, if you wish.
...
The hardware is designed in `Kicad <http://www.kicad-pcb.org/>`_, however, PDF schematics and
